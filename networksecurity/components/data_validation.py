...
class DataValidation:
    ...
    def validate_no_of_cols(self,dataframe:pd.DataFrame)-> bool:
        try:
            ...
            logging.info(f"The number of columns in the dataframe is: {no_of_cols}")
            logging.info(f"The number of columns in the dataframe is: {dataframe.shape[1]}")
    ...
    def detect_dataset_drift(self,base_df,current_df,threshold=0.05)-> bool:
        try:
            ...
            for col in base_df.columns:
                d1 = base_df[col]
                d2 = current_df[col]
                is_different = ks_2samp(d1,d2)
                if threshold <= is_different.pvalue:
                    is_found = False
                else:
                    is_found = True
                    status =False
                report.update({col:{"p_value":float(is_different.pvalue),
    ...
    def initate_data_validation(self) -> DataValidationArtifact:
        try:
            print(type(self.data_ingestion_artifact))
            ...
            status= self.validate_no_of_cols(train_df)
            if not status:
                error_message = f"Train dataframe does not contain all the columns"
            status = self.validate_no_of_cols(test_df)
            if not status:
